...
function [out] = pst_loadspec_rda(filename)
    ...
    fids = zeros(rda.VectorSize,rda.CSIMatrix_Size(1) * rda.CSIMatrix_Size(2) *rda.CSIMatrix_Size(3));
    ...
    if length(filename) >= rawAverages
        if contains(seqtype, 'edit')
            rawAverages = length(filename);
